...
export async function getSteamMedia(
  ...
  const allSteamGames = await Steam.allGames();
  const steamId = knownId !== undefined ? String(knownId) : undefined;
  const steamGame =
    allSteamGames.find((g) => g.gamePath === gamePath) ??
    (steamId !== undefined ? allSteamGames.find((g) => String(g.appid) === steamId) : undefined);
  if (!steamGame) {
    return res;
  }
  const resolvedSteamId = steamId ?? String(steamGame.appid);
