...
class DownloadView extends ComponentEx<IDownloadViewProps, IComponentState> {
  ...
  public shouldComponentUpdate(nextProps: IDownloadViewProps, nextState: IComponentState) {
    ...
    return (
      this.props.downloads !== nextProps.downloads ||
  ...
  private pausable = (downloadIds: string[]) => {
    return (
      downloadIds.find((downloadId: string) => {
        const download = this.getDownload(downloadId);
        return download.state === "started" && download.pausable !== false;
      }) !== undefined
    );
  };
  ...
  private reportDownloadError(err: any, resume: boolean) {
    ...
    if (err instanceof ProcessCanceled) {
      ...
      this.context.api.showErrorNotification(title, err.message, {
        allowReport: false,
      });
      ...
      this.context.api.showErrorNotification(
        ...
        "Nexus Mods does not use self-signed certificates. This error signifies that " +
          ...
          "network connection before trying again.",
        { allowReport: false },
      );
    ...
    onShowDialog(
      ...
      "Confirm Deletion",
      {
        text: t("Do you really want to delete this archive?", {
          ...
          replace: { count: downloadIds.length },
        }),
  ...
  private open = (downloadId: string) => {
    ...
    if (download?.localPath !== undefined) {
      ...
      const internalGameId = rawGame
        ? convertGameIdReverse(games as any, rawGame) || rawGame
        : rawGame;
  ...
  private openModPage = (dlId: string) => {
    ...
    const url =
      path.join("www.nexusmods.com", ids.gameId, "mods", ids.modId.toString()) +
      `?tab=files&file_id=${ids.fileId}&nmm=1`;
