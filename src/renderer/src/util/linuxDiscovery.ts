...
interface IHeroicGameStoreEntry extends IGameStoreEntry {
...
async function entryFromInfoFile(infoPath: string): Promise<IGameStoreEntry | undefined> {
...
async function entriesFromHeroic(home: string): Promise<IGameStoreEntry[]> {
...
async function entriesFromScanRoots(scanRoots: string[]): Promise<IGameStoreEntry[]> {
  const entries: IGameStoreEntry[] = [];
...
function dedupeEntries(entries: IGameStoreEntry[]): IGameStoreEntry[] {
  const byAppId = new Map<string, IGameStoreEntry>();
  for (const entry of entries) {
